    ...
    >
      <div className="relative">
        <div style={{ paddingBottom: compact ? '70%' : '56.25%' }} className="relative overflow-hidden bg-gray-100">
          ...
          <div className="absolute bottom-2 left-2">
            <span className={`pill ${displayStatus === 'Available' || displayStatus === 'Hiring' ? 'bg-primary/90' : 'bg-blue-500/90'} text-white`}>
              {displayStatus}
            </span>
          </div>
        </div>
      ...
      <CardContent className={`p-3 ${compact ? 'space-y-1' : 'space-y-2'}`}>
        <div className="flex justify-between items-start">
          <h3 className={`font-medium text-balance leading-tight ${compact ? 'text-sm' : 'text-base'}`}>
            {title}
          </h3>
          <TooltipProvider>
            <Tooltip>
        ...
        <div className={`flex justify-between items-center text-xs text-muted-foreground ${compact ? 'mt-1' : ''}`}>
