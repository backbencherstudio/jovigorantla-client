...
import { Star, Plus, UserRound, LogOut, Search, Bell, MessageCircle, Info, Users } from 'lucide-react';
    ...
    <UnreadMessagesContext.Provider value={{ unreadMessages, setUnreadMessages }}>
      <header className="bg-white px-4 md:px-6 border-b sticky top-0 z-20 shadow-sm py-[13px]">
        <div className="max-w-full mx-auto flex items-center justify-between">
          ...
          {!isMobile && <form onSubmit={handleSearch} className="flex-1 max-w-md mx-4 relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
              <Input type="text" placeholder="Search" value={searchQuery} onChange={handleSearchChange} className="pl-9 pr-4 py-2 rounded-full bg-gray-100 border-none h-10" />
            </form>}

          {/* Location - Only on Tablet and Desktop */}
          {!isMobile && <div className="hidden md:flex items-center mr-4">
              <LocationSelector />
            </div>}
          ...
          <div className="flex items-center gap-1 md:gap-3">
            ...
            {isEmployee && (
              ...
              >
                <Users className={`text-brand ${isMobile ? 'h-6 w-6' : 'h-5.5 w-5.5'}`} />
              </Button>
            )}
              
            {/* Post button with plus icon - Reduced size */}
            <Button onClick={handlePostAd} className="bg-brand hover:bg-brand/90 text-white flex items-center gap-1 rounded-full px-3 py-1.5 h-8 md:h-8 text-xs md:text-xs">
            ...
            {user && (
              <div className="relative">
                <Button variant="ghost" size="icon" className="rounded-full h-9 w-9" onClick={() => navigate('/messages')}>
                  <MessageCircle className={`text-brand ${isMobile ? 'h-6 w-6' : 'h-5.5 w-5.5'}`} />
                </Button>
                {unreadMessages > 0 && (
                  <Badge 
                    className="absolute -top-1 -right-1 h-4 min-w-4 p-0 flex items-center justify-center text-[9px] bg-[#bf072c] border-white border"
                  >
                    {unreadMessages}
                  </Badge>
                )}
            ...
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="rounded-full bg-gray-100 w-8 h-8 md:w-8 md:h-8">
                    <UserRound className={isMobile ? 'h-6 w-6 text-brand' : 'h-5 w-5 text-brand'} />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem onClick={() => navigate('/saved-listings')}>
