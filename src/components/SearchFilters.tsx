...
const SearchFilters = ({ onCategoryChange, onRadiusChange }: SearchFiltersProps) => {
    ...
    <div className="flex items-center gap-1.5 overflow-x-auto py-1.5 scrollbar-hide bg-transparent">
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="h-7 gap-1 border-gray-200 text-xs md:h-6 md:text-xs md:px-2">
        ...
        {categories.slice(0, 5).map((category) => (
          <Button
            ...
            className={`h-7 text-xs whitespace-nowrap px-3 md:h-6 md:text-xs md:px-2.5 md:py-0.5 font-medium ${
              selectedCategory === category ? 'bg-brand hover:bg-brand/90' : 'border-gray-200'
            }`}
            onClick={() => handleCategorySelect(category)}
          >
            {category}
