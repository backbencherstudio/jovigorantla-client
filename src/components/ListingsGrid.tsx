...
type ListingOrAd = 
  ...
  | { isAd: true; adData: { id: string; name?: string; imageUrl: string; targetUrl: string; group?: string } };
...
const ListingsGrid = memo(({ listings, compact = false }: ListingsGridProps) => {
  ...
  const loadMoreListings = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (newListings.length > 0) {
        ...
        const regularListings = newListings.map(listing => ({ isAd: false as const, listing }));
        ...
        regularListings.forEach((listing, index) => {
          ...
          const totalRegularListings = visibleItems.filter(item => !item.isAd).length + index + 1;
    ...
    <div className="space-y-6">
      <div className="grid gap-4 grid-cols-1 max-w-md mx-auto">
        {visibleItems.map((item, index) => {
          ...
          if (item.isAd === true) {
            return (
              <div key={`ad-${item.adData.id}-${index}`} className="h-full w-full">
            ...
            return (
              <div key={`listing-${item.listing.id}`} className="h-full w-full">
      ...
      {(hasMore || loading) && (
        <div ref={loaderRef} className="flex justify-center py-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-muted-foreground">Loading more listings...</span>
          </div>
        </div>
      )}
    </div>
