...
function isInUSA(lat, lng) {
  ...
  const MIN_LNG = -125.0; // Westernmost point (Washington)
  const MAX_LNG = -66.93457; // Easternmost point (Maine)
...
const findNearestCity = (lat: number, lng: number): Location => {
  ...
  if (!isValidCoordinate(lat, lng)) {
    console.warn(
      `Invalid coordinates provided (${lat}, ${lng}), falling back to Dallas`
    );
    return DALLAS_FALLBACK;
  }
...
const LocationWithRadius: React.FC<LocationWithRadiusProps> = ({
  ...
  const [dispalySelectedOption, setDisplaySelectedOption] =
  ...
  const handleUpdate = () => {
    ...
    if (setCities && selectedOption && radius) {
      ...
      setCities(
        getNearbyCities(selectedOption.lat, selectedOption.lng, radius)
      );
              ...
              styles={{
                control: (base, state) => ({
                  ...
                  border: state.isFocused
                    ? "2px solid #ff7a19"
                    : "2px solid #d1d5db", // Change border color on focus
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium">
                {notSetDefault ? "Select" : "Search"} Radius
              </span>
              {/* <Input
                type="text"
                value={displayRadius}
              ...
              /> */}
              <span className="text-sm text-[#FF7A19]">
                {radius || "100"} miles
              </span>
            </div>
