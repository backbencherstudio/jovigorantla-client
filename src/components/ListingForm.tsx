...
//   useEffect(() => {
//     const shouldResetSubCategory = selectedCategory &&
...
//                     <span className="text-xs text-gray-400">{titleLength}/{MAX_TITLE_LENGTH}</span>
//                   </div>
//                 </div>
//               </FormControl>
...
//                     <button type="button" onClick={removeImage} className="absolute top-1 right-1 bg-black bg-opacity-50 rounded-full p-1 text-white">
//                       <X size={16} />
//                     </button>
//                   </div>
//                 ))}
...
//                     <input type="file" accept="image/*" multiple={false} onChange={handleImageChange} className="hidden" disabled={isSubmitting} />
...
//             {showUSAOption && (
//               <FormField control={form.control} name="postToUSA" render={({ field }) => (
...
//                         </TooltipTrigger>
//                         <TooltipContent>
//                           <p className="w-[200px] text-sm">Reviewed by Desieasy team, will go live if approved.</p>
//                         </TooltipContent>
//                       </Tooltip>
//                     </TooltipProvider>
//                     <FormLabel className="font-medium">Also post in USA Listings</FormLabel>
//                   </div>
...
//       <CustomModal type="success" open={isOpenSuccess} onOpenChange={setIsOpenSuccess} title={<div>Awesome! Your listing is successfully posted.</div>} icon={<img className="ml-10 h-40 w-40" src={successImg} alt="success" />} />
//       <CustomModal open={isOpenPending} onOpenChange={setIsOpenPending} title="Your listing is under review and will be live if approved." icon={<img className="" src={loadingImg} alt="loading" />} />
//       <CustomModal open={isOpenError} onOpenChange={setIsOpenError} type="error" title="Oops! An Unexpected error has occurred, Please refresh the page" icon={<img className="h-40 w-40" src={errorImg} alt="error" />} />
//     </div>
//   );
// };
...
const ListingForm = ({
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      ...
      formData.append('cities', JSON.stringify([{ "latitude": 40.7831, "longitude": -73.9712, "address": "Manhattan" }]));
