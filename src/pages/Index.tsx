...
// import ListingsContainer from '@/components/ListingsContainer';
// import FilterTabs from '@/components/FilterTabs';
// import { getFilterTabs, getPageCategory } from '@/utils/listingUtils';

// const Index = () => {
//   const { listings, loading, hasMore, fetchNearByListings, isInitialMount, setSubCategory, setIsUsa } = useListing();
//   const [searchParams, setSearchParams] = useSearchParams();
//   const [activeFilter, setActiveFilter] = useState('Nearby');
//   const location = useLocation();
...
//   return (
//     <div className="w-full pb-0">
//       <FilterTabs tabs={filterTabs} activeTab={activeFilter} onTabClick={handleFilterClick} />
//       <div className="px-4 pt-2">
//         <ListingsContainer
//         listings={listings}
//                   isLoading={loading}
...
const Index = () => {
  const { listings, loading, hasMore, fetchNearByListings, setSubCategory, setIsUsa } = useListing();
  const [searchParams, setSearchParams] = useSearchParams();
  const [activeFilter, setActiveFilter] = useState('Nearby');
  const location = useLocation();
  const loadingRef = useRef<HTMLDivElement | null>(null);
  ...
  }, [hasMore, loading, listings.length]);

  // Restore scroll position after new listings are added
  useEffect(() => {
    if (listings.length > prevListingsLength.current) {
  ...
  return (
    <div className="w-full pb-0" ref={containerRef}>
      <FilterTabs tabs={filterTabs} activeTab={activeFilter} onTabClick={handleFilterClick} />
      <div className="px-4 pt-2">
        <ListingsContainer
          listings={listings}
