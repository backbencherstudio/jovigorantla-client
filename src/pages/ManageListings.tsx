...
const ManageListings = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (storedListings) {
        ...
        localStorage.setItem(`userListings_${user.id}`, JSON.stringify(mockUserListings));
      }
      setIsLoading(false);
    }, 1000);
  }, [user, navigate]);
  ...
  const deleteListing = () => {
    ...
    const updatedListings = listings.filter(listing => listing.id !== listingToDelete);
    setListings(updatedListings);

    // Update localStorage
    if (user) {
      localStorage.setItem(`userListings_${user.id}`, JSON.stringify(updatedListings));
    }
