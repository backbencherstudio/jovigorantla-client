...
export default function Home({ openModal }) {
  ...
  // Scroll to last viewed listing after listings are rendered
  useEffect(() => {
    if (isTabChanging || isInitialLoad) return;
    const lastId = sessionStorage.getItem("lastViewedListingId");
    if (lastId) {
      setTimeout(() => {
        const el = document.querySelector(`[data-listing-id='${lastId}']`);
        if (el) {
          el.scrollIntoView({ behavior: "auto", block: "center" });
        }
        sessionStorage.removeItem("lastViewedListingId");
      }, 0);
    }
  }, [listings, isTabChanging, isInitialLoad]);
  ...
  return (
    ...
    >
      ...
      {!isTabChanging ? (
        <div className="px-4 my-4 space-y-4">
          {listings.map((listing, index) => (
            <div key={`${listing.id}-${index}`} data-listing-id={listing.id}>
              {listing?.type === "listing" && (
                <ListingItem
                  listing={listing}
                  onToggleSave={() => {}}
