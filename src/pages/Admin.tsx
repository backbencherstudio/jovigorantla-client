...
import { Trash2, Eye, Ban, Flag, Image, UploadCloud, PlusCircle, Save, X, Edit, ExternalLink, Check, ListFilter, History } from 'lucide-react';
...
const Admin = () => {
  ...
  const [sidebarTopPreview, setSidebarTopPreview] = useState<string | null>(null);
  const [sidebarBottomPreview, setSidebarBottomPreview] = useState<string | null>(null);
  const [sidebarTopUrl, setSidebarTopUrl] = useState<string>('https://example.com');
  const [sidebarBottomUrl, setSidebarBottomUrl] = useState<string>('https://example.com');
  ...
  const handleSidebarTopFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
  ...
  const handleSidebarBottomFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
  ...
  // Preview ad before saving
  const previewAd = () => {
    if (newAdForm.isAddingToExistingGroup) {
      const imageUrl = adPreview || `https://via.placeholder.com/600x400?text=${encodeURIComponent(newAdForm.name)}`;
      setPreviewAdData({
        name: newAdForm.name,
        imageUrl,
      ...
      // For ads created with new groups
      const imageUrl = adPreview || `https://via.placeholder.com/600x400?text=${encodeURIComponent(newAdForm.name)}`;
      setPreviewAdData({
        name: newAdForm.name,
        imageUrl,
  ...
  const previewSidebarTopAd = () => {
    const imageUrl = sidebarTopPreview || `https://via.placeholder.com/300x600?text=Top+Sidebar`;
    setPreviewAdData({
      name: "Sidebar Top Ad",
      imageUrl,
  ...
  const previewSidebarBottomAd = () => {
    const imageUrl = sidebarBottomPreview || `https://via.placeholder.com/300x600?text=Bottom+Sidebar`;
    setPreviewAdData({
  ...
  // Handle creating or editing ad group
  const handleSaveAdGroup = () => {
    ...
    if (editingGroupId) {
      ...
      setAdGroups(groups => groups.map(group => 
      ...
      // If there's a new ad to add with this group
      if (newAdForm.name && newAdForm.targetUrl && (selectedFile || adPreview)) {
        const imageUrl = adPreview || `https://via.placeholder.com/600x400?text=${encodeURIComponent(newAdForm.name)}`;
  ...
  // Handle creating new ad for an existing group
  const handleSaveAdToExistingGroup = () => {
    ...
    // For this demo, we'll use the preview URL or a placeholder
    const imageUrl = adPreview || `https://via.placeholder.com/600x400?text=${encodeURIComponent(newAdForm.name)}`;
    ...
    const newAd: Ad = {
      ...
      imageUrl,
      targetUrl: newAdForm.targetUrl,
      order: 0, // Will be set based on group's existing ads
      createdAt: new Date(),
      views: 0,
      clicks: 0,
      active: true
  ...
  // Handle toggling ad group active status
  const handleToggleGroupActive = (groupId: string) => {
    ...
    if (group) {
      toast.success(`Ad group "${group.name}" ${group.active ? 'deactivated' : 'activated'}`);
    }
  };
  ...
  const handleToggleAdActive = (groupId: string, adId: string) => {
    ...
    if (ad) {
      toast.success(`Ad "${ad.name}" ${ad.active ? 'deactivated' : 'activated'}`);
    }
  };
  
  // Handle toggling sidebar ad active status
  const handleToggleSidebarAdActive = (position: 'top' | 'bottom') => {
    adService.toggleSidebarAdActive(position);
    toast.success(`${position.charAt(0).toUpperCase() + position.slice(1)} sidebar ad toggled`);
  ...
  // Start adding a new ad to an existing group
      ...
      <Tabs defaultValue="ads">
            ...
            <>
              {flaggedListings.length === 0 ? (
                ...
                flaggedListings.map(listing => (
                  <Card key={listing.id} className="mb-4 cursor-pointer hover:shadow-md" onClick={() => handleViewListing(listing.listingId)}>
                    <CardHeader>
        ...
        <TabsContent value="ads">
          {!createMode && !newAdForm.isAddingToExistingGroup ? (
            <div className="space-y-6">
              ...
              <Tabs defaultValue="groups">
                <TabsList className="w-full mb-4">
                  <TabsTrigger value="groups" className="flex-1">Ad Groups</TabsTrigger>
                  <TabsTrigger value="sidebar" className="flex-1">Sidebar Ads</TabsTrigger>
                </TabsList>
                        ...
                        <Card key={group.id} className="overflow-hidden">
                              ...
                              <div>
                                ...
                                <CardDescription className="mt-1">
                                  {group.pages.map(pageId => {
                                    ...
                                    return page ? (
                                      <span key={pageId} className="mr-2 inline-block bg-gray-100 text-gray-700 text-xs px-2 py-0.5 rounded">
                                        {page.label}
                                      </span>
                                    ) : null;
                                  })}
                          ...
                          <CardContent className="pt-4">
                            <div className="text-sm text-gray-500 grid grid-cols-2 gap-2 mb-3">
                              <div>
                                <span className="font-medium text-gray-700">Rotation:</span> {group.rotationMode}
                              </div>
                              <div>
                                <span className="font-medium text-gray-700">Frequency:</span> Every {group.frequency} listings
                              </div>
                              {group.startDate && (
                                <div>
                                  <span className="font-medium text-gray-700">Start:</span> {format(group.startDate, 'MMM d, yyyy')}
                                </div>
                              )}
                              {group.endDate && (
                                <div>
                                  <span className="font-medium text-gray-700">End:</span> {format(group.endDate, 'MMM d, yyyy')}
                                </div>
                              )}
                            </div>
                            ...
                            <div className="mb-3">
                              <div className="flex justify-between items-center mb-2">
                                <h3 className="font-medium">Ads in this group ({group.ads.length})</h3>
                                <Button 
                                  size="sm" 
                                  variant="outline"
                                          ...
                                          <div className="flex justify-between">
                                            ...
                                            <div className="flex space-x-1">
                                              <Button 
                                                ...
                                                onClick={() => handleToggleAdActive(group.id, ad.id)}
                                              >
                                                {ad.active ? (
                          ...
                          <div className="flex items-center gap-4">
                            <div className="border rounded p-2 cursor-pointer hover:bg-gray-50 flex-grow">
                              <Input 
                                ...
                                className="hidden"
                              />
                              <Label htmlFor="topAdImage" className="flex items-center justify-center cursor-pointer">
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                          ...
                          <div className="flex items-center gap-4">
                            <div className="border rounded p-2 cursor-pointer hover:bg-gray-50 flex-grow">
                              <Input 
                                ...
                                className="hidden"
                              />
                              <Label htmlFor="bottomAdImage" className="flex items-center justify-center cursor-pointer">
                                <UploadCloud className="h-5 w-5 mr-2 text-gray-400" />
                                <span>Select Image</span>
                              </Label>
                            </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex space-x-2">
                            <Button variant="outline" onClick={previewSidebarBottomAd}>
                              <Eye className="h-4 w-4 mr-2" />
                              Preview
                            </Button>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Button 
                  ...
                  size="sm" 
                  onClick={() => {
                    setNewAdForm({...newAdForm, isAddingToExistingGroup: false, groupId: null});
                    setSelectedFile(null);
                    setAdPreview(null);
                  }}
                >
              ...
              <Card>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="border rounded p-2 cursor-pointer hover:bg-gray-50 flex-grow">
                          <Input 
                            ...
                            className="hidden"
                          />
                          <Label htmlFor="adImage" className="flex items-center justify-center cursor-pointer">
                            <UploadCloud className="h-5 w-5 mr-2 text-gray-400" />
                            <span>Select Image</span>
                          </Label>
                        </div>
                ...
                <CardFooter className="flex justify-between">
                  <Button variant="outline" onClick={previewAd} disabled={!newAdForm.name.trim() || !newAdForm.targetUrl.trim()}>
                    <Eye className="h-4 w-4 mr-2" />
                    Preview
                  </Button>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-semibold">{editingGroupId ? 'Edit Ad Group' : 'Create Ad Group'}</h2>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="grid gap-2">
                        ...
                        <Select 
                          ...
                          onValueChange={(value: 'sequential' | 'random') => updateFormField('rotationMode', value)}
                        >
                          <SelectTrigger>
                      ...
                      <div className="grid gap-2">
                        <Label htmlFor="frequency">Frequency (show every X listings)</Label>
                        <Input 
                          id="frequency" 
                          type="number" 
                          min="1"
                          value={formState.frequency} 
                          onChange={(e) => updateFormField('frequency', parseInt(e.target.value) || 1)} 
                        />
                      </div>
                    </div>
                    ...
                    {!editingGroupId && (
                      <div className="border-t pt-4 mt-4">
                        <h3 className="font-medium mb-3">Create First Ad (Optional)</h3>
                        <div className="space-y-4">
                          <div className="grid gap-2">
                            ...
                            <div className="flex items-center gap-4">
                              <div className="border rounded p-2 cursor-pointer hover:bg-gray-50 flex-grow">
                                <Input 
                                  ...
                                  className="hidden"
                                />
                                <Label htmlFor="firstAdImage" className="flex items-center justify-center cursor-pointer">
                                  <UploadCloud className="h-5 w-5 mr-2 text-gray-400" />
                                  <span>Select Image</span>
                                </Label>
                              </div>
          ...
          {!showUsaHistory && (
            <>
              {usaListings.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-lg">
                  <ListFilter className="w-12 h-12 mx-auto text-gray-400 mb-2" />
                  <p className="text-gray-500">No USA listings waiting for approval</p>
                </div>
              ) : (
                <div>
                  <Table>
                    ...
                    <TableBody>
                      {usaListings.map(listing => (
                        <TableRow key={listing.id}>
                          ...
                          <TableCell>{listing.postedBy}</TableCell>
                              ...
                              {listing.status === 'pending' && (
                                <>
                                  <Button
                                    ...
                                    onClick={() => handleApproveUSAListing(listing.id)}
                                  >
                                    <Check className="h-4 w-4 mr-1" />
                                    Approve
                                  </Button>
          ...
          {previewAdData && (
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 break-all mt-1">{previewAdData.targetUrl}</p>
              </div>
              <Button
                variant="outline"
                size="sm"
