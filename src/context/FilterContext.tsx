...
export const FilterProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [nextCursorDistance, setNextCursorDistance] = useState<number | null>(
    null
  );
  const [nextListingOffset, setNextListingOffset] = useState<number | null>(
    null
  );
  ...
  const buildApiParams = (isLoadMore: boolean = false) => {
    ...
    if (
      subCategory &&
      category &&
      !["All", "Nearby", "USA"].includes(subCategory)
    ) {
  ...
  useEffect(() => {
    ...
    if (subcategoryParam && subcategoryParam !== subCategory?.toLowerCase()) {
      const formattedSubCategory =
        subcategoryParam.charAt(0).toUpperCase() +
        subcategoryParam.slice(1).toLowerCase();
      setSubCategory(formattedSubCategory);
    }
